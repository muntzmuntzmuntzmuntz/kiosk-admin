...
function displayStatus(code: ActivationCode) {
  ...
  if (code.deviceId && new Date(code.expiresAt).getTime() < Date.now()) return "expired";
  return code.status;
}
...
function canUpgradeExpiration(code: ActivationCode) {
  const days = Math.ceil((new Date(code.expiresAt).getTime() - Date.now()) / 86400000);
  return code.code.startsWith("ZYR") || (days >= 0 && days <= EXPIRING_SOON_DAYS);
}
...
function RefreshIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 20 20" className="h-4 w-4 fill-none stroke-current stroke-[1.8]">
      <path d="M16.25 7.25A6.5 6.5 0 0 0 4.8 4.8L3.5 6.1" />
      <path d="M3.5 2.75V6.1h3.35" />
      <path d="M3.75 12.75A6.5 6.5 0 0 0 15.2 15.2l1.3-1.3" />
      <path d="M16.5 17.25V13.9h-3.35" />
    </svg>
  );
}
...
export default function AdminPage() {
  ...
  const fetchCodes = useCallback(
    async (params: { page: number; status: string; device: string }, signal?: AbortSignal) => {
      ...
      if (params.device !== "all") query.set("device", params.device);

      const res = await fetch(`/api/activation-codes/list?${query.toString()}`, { signal });
      if (!res.ok) throw new Error("Failed to load activation codes");
  ...
  async function removeCode(code: string) {
    if (!window.confirm("Remove this activation code permanently?")) return;

    setRemovingCode(code);
    setErrorMessage(null);

    try {
      const res = await fetch(`/api/activation-codes/${code}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to remove activation code");
      await refreshCodes();
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Something went wrong");
    } finally {
  ...
  async function saveDescription() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to update activation code");
  ...
  const activePercentage = totalCount ? Math.round((activeCodes / totalCount) * 100) : 0;
  const revokedPercentage = totalCount ? Math.round((revokedCodes / totalCount) * 100) : 0;
  const assignedPercentage = totalCount ? Math.round((assignedCodes / totalCount) * 100) : 0;
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-950">
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            { label: "Total", value: totalCount, detail: "All generated codes" },
            ...
            <article key={stat.label} className="border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div className="flex min-h-16 flex-col gap-4 border-b border-zinc-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              >
                <button
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center border border-zinc-300 bg-white text-zinc-700 shadow-sm transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={loadState === "loading" || isCreating}
                  aria-label="Refresh activation codes"
                  title="Refresh"
                >
                  <RefreshIcon />
                </button>
                <button
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center bg-zinc-950 text-lg font-semibold text-white shadow-sm transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  +
                </button>
                {isCreateMenuOpen ? (
                  <div role="menu" className="absolute right-0 z-20 mt-2 w-36 overflow-hidden border border-zinc-200 bg-white py-1 shadow-lg">
                    <button
                      type="button"
                      role="menuitem"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
                <label className="flex flex-col text-sm font-medium text-zinc-700">
                  ...
                  <select
                    ...
                    className="h-10 border border-zinc-300 bg-white px-3 text-sm text-zinc-900 shadow-sm transition focus:border-zinc-500 focus:outline-none"
                  >
                      ...
                      return (
                        <tr key={code.id} className="hover:bg-zinc-50">
                          <td className="px-5 py-4">
                            <span className="font-mono text-sm font-medium text-zinc-950">{code.code}</span>
                          </td>
                          <td className="px-5 py-4">
                            <span className={`inline-flex h-7 items-center border px-3 text-xs font-semibold capitalize ${statusClass(status)}`}>
                              {status}
                            </span>
                          </td>
                          ...
                          <td className="px-5 py-4 text-zinc-600">
                            {code.deviceId ? <span className="font-mono text-xs">{code.deviceId}</span> : <span className="text-zinc-400">Unassigned</span>}
                          </td>
                          <td className="px-5 py-4 text-zinc-600">
                            {code.description ? (
                          ...
                          <td className="px-5 py-4 text-right">
                            <div className="inline-flex items-center gap-2">
                              <button
                                ...
                                className="inline-flex h-9 w-9 items-center justify-center border border-zinc-300 bg-white text-sm font-medium text-zinc-700 transition hover:bg-zinc-100"
                              >
                                <PencilIcon />
                              </button>
                              <button
                                ...
                                className="inline-flex h-9 items-center justify-center border border-zinc-300 bg-white px-3 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:border-zinc-200 disabled:text-zinc-400"
                              >
            ...
            <div className="flex flex-col gap-3 border-t border-zinc-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 1}
                  className="inline-flex h-10 items-center justify-center border border-zinc-300 bg-white px-4 text-sm font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Previous
                </button>
                <button
      ...
      {editingCode ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 px-4">
          <div className="w-full max-w-lg border border-zinc-200 bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium text-zinc-500">Edit activation code</p>
                <h2 className="mt-1 font-mono text-sm font-semibold text-zinc-950">{editingCode.code}</h2>
              </div>
              <button
                type="button"
                onClick={closeDescriptionEditor}
                className="p-2 text-zinc-500 transition hover:bg-zinc-100 hover:text-zinc-700"
            ...
            <label className="mt-5 block text-sm font-medium text-zinc-700">
              ...
              <textarea
                ...
                rows={4}
                maxLength={200}
                className="mt-2 w-full border border-zinc-300 bg-white px-4 py-3 text-sm text-zinc-900 shadow-sm outline-none transition focus:border-zinc-500"
                placeholder="Add a note for this code, device, or deployment."
              />
            </label>

            <p className="mt-2 text-right text-xs text-zinc-400">{descriptionDraft.length}/200</p>
            ...
            <div className="mt-5 border border-zinc-200 bg-zinc-50 px-4 py-3">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="text-sm text-zinc-600">
                  {editingCode.deviceId ? dateFormatter.format(new Date(editingCode.expiresAt)) : "Starts on assignment"}
                </span>
              </div>
              ...
              {showUpgradeToggle ? (
                <label className="mt-4 flex items-center justify-between gap-4">
                  <span>
                    <span className="block text-sm font-semibold text-zinc-800">Upgrade expiration</span>
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button
                ...
                onClick={closeDescriptionEditor}
                disabled={isSavingDescription}
                className="h-10 border border-zinc-300 bg-white px-4 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isSavingDescription}
                className="h-10 bg-zinc-950 px-4 text-sm font-semibold text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-60"
              >
